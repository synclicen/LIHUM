...
export default function AdminPanel({
  ...
  const handleAddAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(`Akun ${newEmail} sukses terdaftar sebagai ${newRole === "admin" ? "Admin" : "Manajer"}!`);
      setNewEmail("");
      setNewDisplayName("");
      setNewRole("manager");
      loadAccounts();
  ...
  return (
    <div id="admin-panel-container" className="space-y-6">
      ...
      {userRole === "admin" && (
        <div id="admin-tab-bar" className="flex border-b border-[#D4AF37]/15 pb-2 ml-1 mb-2 space-x-6">
          <button
            type="button"
            onClick={() => {
          ...
          >
            ...
            <span>Manajemen Akun ({accounts.length ? accounts.length : "..."})</span>
          </button>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 bg-[#120A21] border border-violet-900/40 rounded-2xl p-6 shadow-2xl relative backdrop-blur-sm self-start">
            <div className="flex items-center space-x-2.5 mb-5 pb-3 border-b border-violet-900/30">
              ...
              <h2 className="text-md font-serif font-bold text-white">Daftarkan Akun Baru</h2>
            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <span className="text-[9px] text-slate-300 text-center font-normal">Buat & Share Galeri</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="text-[9px] text-slate-300 text-center font-normal">Semua + Kelola Akun</span>
                  </button>
                </div>
              </div>
          ...
          <div className="lg:col-span-2 bg-[#120A21]/30 border border-violet-900/20 rounded-2xl p-6 shadow-2xl backdrop-blur-sm self-start">
            <div className="flex items-center justify-between mb-5 pb-3 border-b border-violet-900/35">
              <div className="flex items-center space-x-2.5">
                ...
                <h2 className="text-md font-serif font-bold text-white">Daftar Akun Pengelola Aktif</h2>
              </div>
            ...
            {loadingAccounts ? (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <span className="text-xs text-slate-400 font-mono">Mengunduh daftar akun...</span>
              </div>
            ) : accounts.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-xs text-slate-400">Belum ada akun pengelola yang terdaftar.</p>
              </div>
            ) : (
              <div className="overflow-x-auto w-full">
                <table className="w-full text-left text-xs text-slate-300">
                  ...
                  <tbody className="divide-y divide-violet-950/50">
                    {accounts.map((acc) => {
                      const isOwnAccount = acc.email.toLowerCase() === userEmail.toLowerCase();
                      const isPrimaryAdmin = acc.email.toLowerCase() === "synclicen@gmail.com";

                      return (
                        <tr key={acc.id} className="hover:bg-violet-950/25 transition-colors">
                          <td className="py-3 px-2">
                            <div className="font-semibold text-white">{acc.displayName || "Tanpa Nama"}</div>
                          ...
                          <td className="py-3 px-2 text-right">
                            {isOwnAccount ? (
                              <span className="text-[10px] text-slate-500 font-semibold px-2">Akun Anda</span>
                            ) : isPrimaryAdmin ? (
                              <span className="text-[10px] text-[#D4AF37]/50 font-semibold px-2">Utama</span>
                            ) : (
                              <button
                                type="button"
                                onClick={() => handleDeleteAccount(acc.id, acc.email)}
                                className="p-1.5 rounded-lg border border-violet-950 hover:border-red-650 text-slate-400 hover:text-red-400 hover:bg-red-500/5 transition-all cursor-pointer inline-flex items-center"
                                title="Hapus Akun"
                              >
